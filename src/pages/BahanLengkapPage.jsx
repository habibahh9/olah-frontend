...
export default function BahanLengkapPage() {
  const navigate = useNavigate();
  const location = useLocation();
  const [searchQuery, setSearchQuery] = useState("");
  const [isProfileMenuOpen, setIsProfileMenuOpen] = useState(false);

  const INITIAL_FAVORITE_MENUS = [
    { id: 1,  title: "Tumis Bayam Tahu",    time: "30 Menit", portion: "2 Porsi", ingredients: ["Bayam", "Tahu"],      available: true, isFavorite: true },
    { id: 5,  title: "Sambal Goreng Tempe", time: "45 Menit", portion: "1 Porsi", ingredients: ["Tempe", "Cabe"],      available: true, isFavorite: true },
    { id: 6,  title: "Sate Ayam",           time: "60 Menit", portion: "2 Porsi", ingredients: ["Ayam"],               available: true, isFavorite: true },
    { id: 7,  title: "Telur Balado",        time: "45 Menit", portion: "1 Porsi", ingredients: ["Telur", "Cabe"],      available: true, isFavorite: true },
    { id: 8,  title: "Sup Ayam",            time: "45 Menit", portion: "1 Porsi", ingredients: ["Ayam", "Wortel"],     available: true, isFavorite: true },
    { id: 10, title: "Tumis Tahu",          time: "20 Menit", portion: "2 Porsi", ingredients: ["Tahu", "Kol"],        available: true, isFavorite: true },
    { id: 12, title: "Sambal Tempe",        time: "30 Menit", portion: "1 Porsi", ingredients: ["Tempe"],              available: true, isFavorite: true },
    { id: 13, title: "Sup Bayam",           time: "20 Menit", portion: "2 Porsi", ingredients: ["Bayam"],              available: true, isFavorite: true },
    { id: 14, title: "Telur Ceplok",        time: "10 Menit", portion: "1 Porsi", ingredients: ["Telur"],              available: true, isFavorite: true },
    { id: 16, title: "Ayam Bakar",          time: "50 Menit", portion: "2 Porsi", ingredients: ["Ayam"],               available: true, isFavorite: true },
  ];

  const INITIAL_ALL_MENUS = [
    { id: 6,  title: "Sate Ayam",    time: "60 Menit", portion: "2 Porsi", ingredients: ["Ayam"],           available: true, isFavorite: false },
    { id: 7,  title: "Telur Balado", time: "45 Menit", portion: "1 Porsi", ingredients: ["Telur", "Cabe"],  available: true, isFavorite: false },
    { id: 8,  title: "Sup Ayam",     time: "45 Menit", portion: "1 Porsi", ingredients: ["Ayam", "Wortel"], available: true, isFavorite: false },
    { id: 10, title: "Tumis Tahu",   time: "20 Menit", portion: "2 Porsi", ingredients: ["Tahu", "Kol"],    available: true, isFavorite: false },
    { id: 12, title: "Sambal Tempe", time: "30 Menit", portion: "1 Porsi", ingredients: ["Tempe"],          available: true, isFavorite: false },
    { id: 13, title: "Sup Bayam",    time: "20 Menit", portion: "2 Porsi", ingredients: ["Bayam"],          available: true, isFavorite: false },
    { id: 14, title: "Telur Ceplok", time: "10 Menit", portion: "1 Porsi", ingredients: ["Telur"],          available: true, isFavorite: false },
  ];

  const [favoriteMenus, setFavoriteMenus] = useState(() => {
    const savedFavs = loadStorage("bahanLengkap_favorites", null);
    if (savedFavs) {
      return INITIAL_FAVORITE_MENUS.map((r) => ({
        ...r,
        isFavorite: savedFavs[r.id] ?? r.isFavorite,
      })).filter((m) => m.available);
    }
    return INITIAL_FAVORITE_MENUS.filter((m) => m.available);
  });

  const [allMenus, setAllMenus] = useState(() => {
    const savedFavs = loadStorage("bahanLengkap_favorites", null);
    if (savedFavs) {
      return INITIAL_ALL_MENUS.map((r) => ({
        ...r,
        isFavorite: savedFavs[r.id] ?? r.isFavorite,
      })).filter((m) => m.available);
    }
    return INITIAL_ALL_MENUS.filter((m) => m.available);
  });

  useEffect(() => {
  const favMap = {};
    [...favoriteMenus, ...allMenus].forEach((r) => {
      favMap[r.id] = r.isFavorite;
    });
    saveStorage("bahanLengkap_favorites", favMap);
  }, [favoriteMenus, allMenus]);
  ...
  const toggleFavorite = (id, e) => {
    ...
    setFavoriteMenus((prev) => toggle(prev));
    setAllMenus((prev) => toggle(prev));
  };
  ...
  const RecipeCard = ({ recipe }) => {
    ...
    <article className="relative h-[220px] cursor-pointer" onClick={() => navigate(`/detail-resep/${recipe.id}`)}>
      <img src={imageUrl} alt={recipe.title} className="absolute top-0 left-0 w-full h-[215px] object-cover rounded-[12px]" />
      ...
      <div className="absolute top-[137px] left-[10px] right-[10px]">
        <span className="font-semibold text-white text-sm leading-normal truncate block">{recipe.title}</span>
      </div>
      <div className="absolute top-[148px] right-[8px] flex items-center gap-1">
    ...
    <div className="flex min-h-screen bg-[#f4f4f4]">
      <aside className="hidden md:flex md:fixed left-0 top-0 w-[110px] h-screen bg-white shadow-lg flex-col items-center py-5 gap-7 z-50">
        ...
        {navItems.map((item) => {
          ...
          return (
            <button key={item.label} onClick={() => navigate(item.path)}
              className={`flex flex-col items-center gap-1 transition text-xs font-medium w-full px-2 ${isActive ? "text-[#d06224]" : "text-gray-300 hover:text-[#d06224]"}`}>
              {item.icon}
        ...
        <div className="mt-auto relative">
          ...
          {isProfileMenuOpen && (
            <div className="absolute bottom-16 left-0 w-48 bg-white rounded-xl shadow-xl border border-gray-100 py-2">
              <button onClick={() => navigate("/profil")} className="w-full text-left px-4 py-2 text-sm hover:bg-gray-50">Kelola Profil</button>
              <button onClick={() => navigate("/sandi")} className="w-full text-left px-4 py-2 text-sm hover:bg-gray-50">Ubah Kata Sandi</button>
              <hr className="my-1" />
      ...
      <main className="md:ml-[110px] flex-1 pb-20 md:pb-0">
        ...
        <div className="px-3 md:px-7 py-4 md:py-5">
          <div className="flex flex-col md:flex-row md:items-center gap-3 md:gap-5 mb-5">
            <div className="flex flex-1">
              <input type="text" placeholder="Cari Resep Masakan" value={searchQuery}
