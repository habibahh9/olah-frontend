  ...
  useEffect(() => {
    const fetchResep = async () => {
      ...
      try {
        ...
        // Filter hanya yang bahan lengkap (available: true)
        const available = mapped.filter((r) => r.available);
        const loved =
          JSON.parse(localStorage.getItem("user") || "{}").lovedRecipes || [];
        setFavoriteMenus(available.filter((r) => loved.includes(r.id)));
        setAllMenus(available);
      } catch (err) {
          ...
          <section>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
              {loading ? (
                ...
                <div className="col-span-full text-center text-sm text-gray-500 py-10">
                  Tidak ada resep dengan bahan lengkap
                </div>
              ) : (
