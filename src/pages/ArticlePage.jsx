...
export default function ArticlePage() {
  ...
  const fetchArticles = async () => {
    ...
    try {
      ...
      // Map ke format yang dipakai ArticleCard
      const mapped = list.map((a, i) => ({
        id: i + 1,
        title: a.title,
        category: "Food Waste",
        author: a.source || "Unknown",
        date: a.publishedAt
      ...
      console.error("Gagal fetch artikel:", err.message);
    } finally {
      setLoading(false);
    }
  };
              ...
              {featured && activeCategory === "Semua" && !searchQuery && (
                <section>
                  ...
                  <div
                    ...
                    onClick={() => featured.url ? window.open(featured.url, "_blank") : navigate(`/artikel/${featured.id}`)}
                  >
...
function ArticleCard({ article, onClick }) {
  ...
  return (
    <article
      onClick={() => article.url ? window.open(article.url, "_blank") : onClick()}
