...
export default function CepatPage() {
    ...
    const fetchResep = async () => {
      ...
      try {
        ...
        const mapped = list.map((r) => ({
          ...
          time: r.cookTime ? `${r.cookTime} Menit` : "-",
        ...
        // Filter resep cepat (< 30 menit)
        const cepat = mapped.filter((r) => {
        ...
        setFavoriteMenus(cepat.filter((r) => loved.includes(r.id)));
        setAllMenus(cepat);
      } catch (err) {
  ...
  const navItems = [
    {
      ...
      path: "/dashboard",
      icon: (
        <svg width="22" height="22" viewBox="0 0 46 47" fill="none">
    ...
    },
    {
      ...
      path: "/keranjang",
      icon: (
        <svg width="22" height="22" viewBox="0 0 44 49" fill="none">
    ...
    },
    {
      ...
      path: "/riwayat",
      icon: (
        <svg width="22" height="22" viewBox="0 0 44 39" fill="none">
  ...
  const RecipeCard = ({ recipe }) => {
    ...
    return (
      ...
      >
        ...
        {/* Indikator bahan lengkap */}
        <div className="absolute top-[138px] right-[8px] flex items-center gap-1">
        ...
        {/* Tombol favorit */}
        <button
          className="absolute top-[6px] right-[8px] p-1"
  ...
  return (
    <div className="flex min-h-screen bg-[#f4f4f4]">
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center gap-1 transition text-xs font-medium w-full px-2 ${
                isActive ? "text-[#d06224]" : "text-gray-300 hover:text-[#d06224]"
              }`}
            >
              {item.icon}
      ...
      <main className="md:ml-[110px] flex-1 pb-20 md:pb-0">
        {/* Header — judul diperbaiki menjadi "Cepat" */}
        <div className="h-[70px] md:h-[90px] bg-white shadow-sm flex items-center px-4 md:px-10">
          <h1 className="text-[28px] font-semibold text-[#d06224]">Cepat</h1>
        </div>
        ...
        <div className="px-3 md:px-7 py-4 md:py-5">
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-3 md:gap-5 mb-5">
            ...
            {/* Tombol navigasi ke halaman Bahan Lengkap */}
            <button
              onClick={() => navigate("/bahan-lengkap")}
              className="h-[40px] md:h-[50px] px-8 rounded-full bg-[#9f9b4a] text-white font-medium"
            >
              Bahan Lengkap
            </button>
          </div>
          ...
          <section>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
              {loading ? (
                ...
                <div className="col-span-full text-center text-sm text-gray-500 py-10">
                  Tidak ada resep cepat tersedia
                </div>
              ) : (
        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              key={item.label}
              onClick={() => navigate(item.path)}
              className={`flex flex-col items-center gap-0.5 text-[10px] font-medium px-2 py-1 transition ${
