...
export default function DashboardPage() {
  ...
  const filteredRecipes = useMemo(() => {
    if (activeFilter === "Cepat") return recipeCards.filter(r => parseInt(r.time) <= 20);
    return recipeCards;
  }, [activeFilter, recipeCards]);
  ...
  return (
    <div className="flex min-h-screen bg-[#f4f4f4]">
        ...
        <div className="mt-auto relative">
          ...
          {isProfileMenuOpen && (
            <div className="absolute bottom-14 left-0 w-48 bg-white rounded-xl shadow-xl border border-gray-100 py-2 z-50">
              <button className="w-full text-left px-4 py-2 text-sm text-gray-600 hover:bg-gray-50">Kelola Profil</button>
              <button className="w-full text-left px-4 py-2 text-sm text-gray-600 hover:bg-gray-50">Ubah Kata Sandi</button>
              <hr className="my-1" />
              <button
      ...
      {/* ── KONTEN UTAMA ── */}
      <main className="ml-[110px] flex-1 flex flex-col overflow-hidden min-w-0">
        ...
        <section className="relative h-[220px] overflow-hidden">
          ...
          <div className="absolute top-[28px] left-[40px] w-[400px]">
            ...
            <div className="flex gap-3 mt-3">
              ...
              <button className="w-[105px] h-8 rounded-[25px] border-2 border-white text-white font-normal text-sm">
                Lihat Artikel
              </button>
            </div>
          </div>
          {/* Search bar */}
          <div className="absolute top-[32px] right-8 flex">
            <input
              type="search"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              placeholder="Cari Resep atau Bahan"
              className="w-[300px] px-4 py-2.5 rounded-l-full bg-white/90 text-sm text-gray-700 placeholder-gray-400 focus:outline-none"
            />
            <button className="bg-[#E87722] px-4 rounded-r-full flex items-center justify-center">
        ...
        <div className="px-7 py-5 flex flex-col gap-6 overflow-hidden">
          ...
          <section>
            ...
            {/* Filter tabs */}
            <div className="flex gap-0 mb-5 relative h-[30px] w-[310px]">
              {recommendationFilters.map((f, index) => {
                const leftMap = ["left-0", "left-[103px]", "left-[206px]"];
                const isActive = activeFilter === f;
                return (
                  <button
                    key={f}
                    onClick={() => setActiveFilter(f)}
                    className={`absolute top-0 ${leftMap[index]} w-[100px] h-[30px] rounded-[15px] text-sm font-medium transition ${
                      isActive ? "bg-[#d06224] text-white" : "bg-white text-[#99999980]"
                    }`}
                  >
                    {f}
                  </button>
            ...
            <div className="grid grid-cols-5 gap-3 w-full">
              {filteredRecipes.map((recipe) => (
                <article key={recipe.id} className="relative h-[210px]">
                  <img
                    src={recipe.image}
                    alt={recipe.title}
                    className="absolute top-0 left-0 w-full h-[210px] object-cover rounded-[12px]"
                  />
                  <div className="absolute top-[125px] left-0 w-full h-[85px] bg-[#8a8635cc] rounded-[12px]" />
                  <div className="absolute top-[132px] left-[10px] right-[10px]">
                  ...
                  <div className="absolute top-[180px] left-[10px] flex gap-1.5">
          ...
          <section className="flex gap-5">
            ...
            <div className="flex-1 bg-white rounded-[15px] p-5 shadow-sm">
              <div className="flex justify-between items-center mb-3">
                ...
                <button className="text-sm text-[#d06224] font-semibold">Lihat Semua</button>
              </div>
              <div className="flex gap-4 items-center">
                ...
                <div className="flex flex-col items-center gap-2 shrink-0">
                  <svg width="150" height="150" viewBox="0 0 150 150">
                    <g transform="rotate(-90 75 75)">
                      <circle cx="75" cy="75" r="52" fill="none" stroke="#ae431e" strokeWidth="26"
                        strokeDasharray="40 287" strokeDashoffset="-2" />
                      <circle cx="75" cy="75" r="52" fill="none" stroke="#d06224" strokeWidth="26"
                        strokeDasharray="197 130" strokeDashoffset="-47" />
                      <circle cx="75" cy="75" r="52" fill="none" stroke="#8a8635" strokeWidth="26"
                        strokeDasharray="79 248" strokeDashoffset="-249" />
                    </g>
                    <circle cx="75" cy="75" r="38" fill="#f5f5f5" />
                    <text x="75" y="75" textAnchor="middle" dominantBaseline="central"
                      fontSize="18" fontWeight="300" fontFamily="Poppins, sans-serif" fill="#222">8</text>
                    <text x="97" y="27" textAnchor="middle" dominantBaseline="central"
                      fontSize="10" fontFamily="Poppins, sans-serif" fontWeight="700" fill="#fff">1</text>
                    <text x="75" y="126" textAnchor="middle" dominantBaseline="central"
                      fontSize="10" fontFamily="Poppins, sans-serif" fontWeight="700" fill="#fff">5</text>
                    <text x="38" y="38" textAnchor="middle" dominantBaseline="central"
                      fontSize="10" fontFamily="Poppins, sans-serif" fontWeight="700" fill="#fff">2</text>
                  </svg>
                  <div className="flex flex-col gap-1">
                    <div className="flex items-center gap-2">
                      <span className="w-2.5 h-2.5 rounded-full bg-[#d06224] inline-block" />
                      <span className="text-[#d06224] text-xs font-semibold">Sayuran</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="w-2.5 h-2.5 rounded-full bg-[#8a8635] inline-block" />
                      <span className="text-[#8a8635] text-xs font-semibold">Protein</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="w-2.5 h-2.5 rounded-full bg-[#ae431e] inline-block" />
                      <span className="text-[#ae431e] text-xs font-semibold">Lainnya</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-2 flex-1 ml-4">
                  {expiringItems.map((item) => (
                    <div key={item.name} className="w-full h-[26px] bg-[#d06224] rounded-full flex items-center px-3">
                      <span className="text-white text-xs font-medium w-9 shrink-0">{item.name}</span>
                      <div className="flex-1 h-2 bg-white/75 rounded-full overflow-hidden mx-2">
                        <div
                          className="h-full bg-[#8a8635] rounded-full"
                          style={{ width: `${item.percent}%` }}
                        />
                      </div>
                      <span className="text-white text-xs font-light shrink-0">{item.days}</span>
                    </div>
                  ))}
                </div>
            ...
            {/* Notifikasi */}
            <div className="w-[280px] bg-white rounded-[15px] p-5 shadow-sm">
