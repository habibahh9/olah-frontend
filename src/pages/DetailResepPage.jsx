...
import { recipeAPI, shoppingListAPI, riwayatAPI, pantryAPI } from "../utils/api";
...
export default function DetailResepPage() {
  ...
  useEffect(() => {
    const fetchRecipeDetail = async () => {
      ...
      try {
        ...
        const [pantryRes, cartRes] = await Promise.all([
          pantryAPI.getAll(),
          shoppingListAPI.getAll(),
        ]);

        const pantryNames = (pantryRes?.data?.pantry ?? [])
          .map(i => i.name.toLowerCase().trim());

        const cartNames = (cartRes?.data?.shoppingList ?? [])
          .map(i => i.name.toLowerCase().trim());

        setCartItems(
          normalized.ingredients.map((ing) => {
            const name = (typeof ing === "string" ? ing : ing.name || "").toLowerCase().trim();
            const inPantry = pantryNames.some(p => name.includes(p) || p.includes(name));
            const inCart = cartNames.some(c => name.includes(c) || c.includes(name));
            // true = ada di pantry/keranjang (putih), false = tidak ada (orange)
            return inPantry || inCart;
          })
        );
      } catch (err) {
  ...
  if (isCurrentlyInCart) return; // sudah ada, tidak perlu aksi
  ...
  // Optimistic update
  setCartItems((prev) => prev.map((v, i) => (i === index ? true : v)));

  try {
    await shoppingListAPI.addItem({ name: ingredientName });
    setToast(`"${ingredientName}" berhasil dimasukkan ke keranjang!`);
    setTimeout(() => setToast(null), 2500);
  } catch (err) {
    // Rollback jika gagal
    setCartItems((prev) => prev.map((v, i) => (i === index ? false : v)));
  ...
  return (
    <PageLayout>
                ...
                return (
                  <div
                    ...
                    className={`flex items-center h-12 rounded-[12px] px-4 transition-colors duration-300 shadow-sm ${
                      cartItems[index] ? "bg-white border border-gray-100" : "bg-[#d06224]"
                    }`}
                  >
                    {/* Nama bahan */}
                    <span
                      className={`flex-1 text-sm font-medium capitalize transition-colors ${
                      cartItems[index] ? "text-gray-700" : "text-white"
                    }`}
                    >
                      {name}
                    </span>
                    ...
                    {quantity && (
                      <span
                        className={`text-xs font-semibold capitalize px-2 py-0.5 rounded-full mr-3 ${
                          cartItems[index] ? "bg-white border border-gray-200 hover:border-[#d06224]" : "bg-[#d06224] shadow-inner"
                        }`}
                      >
                        {quantity} {unit}
                      </span>
                    ...
                    {/* Tombol keranjang */}
                    <button
                    onClick={() => !cartItems[index] && toggleCart(index, item)}
                    disabled={cartItems[index]}
                  ...
                  >
                    <svg width="18" height="18" viewBox="0 0 44 49" fill="none">
                      <path
                        ...
                        fill={cartItems[index] ? "#aaaaaa" : "#d06224"}
                      />
                    </svg>
                  </button>
                  </div>
      ...
      {/* Toast notifikasi */}
      {toast && (
        <div className="fixed bottom-24 md:bottom-8 left-1/2 -translate-x-1/2 z-[200] bg-[#d06224] text-white text-sm font-medium px-5 py-3 rounded-full shadow-lg animate-fade-in">
          🛒 {toast}
        </div>
      )}
    </PageLayout>
