...
export default function DetailResepPage() {
  ...
  useEffect(() => {
    const fetchRecipeDetail = async () => {
      ...
      try {
        ...
        const boughtNames = (cartRes?.data?.shoppingList ?? [])
          .filter(i => i.checked)
          .map(i => i.name.toLowerCase());
        ...
        setCartItems(
          normalized.ingredients.map((ing) => {
            ...
            const inCart = boughtNames.some(b => lower.includes(b) || b.includes(lower));
  ...
  const toggleCart = async (index, ingredient) => {
    ...
    // Kalau sudah putih karena pantry, tidak perlu tambah ke keranjang
    // Langsung optimistic update dulu
    setCartItems((prev) => prev.map((v, i) => (i === index ? !v : v)));

    try {
      if (!isCurrentlyInCart) {
        await shoppingListAPI.addItem({ ingredientName });
        setToast(`"${ingredientName}" ditambahkan ke keranjang!`);
                ...
                return (
                  <div
                    ...
                    className={`flex items-center h-12 rounded-[12px] px-4 transition-colors duration-300 shadow-sm ${
                      cartItems[index] ? "bg-white" : "bg-[#ae431e]"
                    }`}
                  >
                    {/* Nama bahan */}
                    <span
                      className={`flex-1 text-sm font-medium capitalize transition-colors ${
                        cartItems[index] ? "text-[#ae431e]" : "text-white"
                      }`}
                    >
                      {name}
                    </span>
                    ...
                    >
                      <svg width="18" height="18" viewBox="0 0 44 49" fill="none">
                        <path
                          ...
                          fill={cartItems[index] ? "#d06224" : "white"}
                        />
                      </svg>
                    </button>
                  </div>
