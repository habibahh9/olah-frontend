...
// Data riwayat dikelompokkan per bulan
const riwayatData = [
  {
    bulan: "April 2026",
    items: [
...
// Leaderboard resep paling sering dimasak
const resepSering = [
...
export default function RiwayatPage() {
  ...
  const [filter, setFilter] = useState("Semua"); // Semua | Bulan Ini | Minggu Ini

  const filterOptions = ["Semua", "Bulan Ini", "Minggu Ini"];

  // Filter riwayat berdasarkan search dan tab
    ...
    <div className="flex min-h-screen bg-[#f4f4f4]">
      ...
      <main className="ml-[110px] flex-1">
        ...
        <div className="h-[90px] bg-white shadow-sm flex items-center px-10">
          <h1 className="text-[28px] font-bold text-[#d06224]">Riwayat Masakan</h1>
        </div>

        {/* CONTENT */}
        <div className="px-7 py-5 flex gap-6">
              ...
              <div className="flex gap-2">
                {filterOptions.map((opt) => (
                  <button
                    ...
                    className={`h-[40px] px-6 rounded-full text-sm font-medium transition ${
                      filter === opt
            ...
            {filteredData.length === 0 ? (
              ...
              filteredData.map((group) => (
                <section key={group.bulan}>
                  {/* Heading bulan — sama dengan h2 di ResepPage */}
                  <h2 className="text-lg font-light text-black mb-4">{group.bulan}</h2>

                  <div className="flex flex-col gap-3">
                    {group.items.map((item) => (
                      <div
                        key={item.id}
                        className="bg-white rounded-[12px] px-6 py-4 flex items-center gap-4 shadow-sm"
                      >
                        {/* Info kiri */}
                        <div className="flex-1">
                          <p className="font-semibold text-[#d06224] text-base mb-1">{item.title}</p>
                          <div className="flex items-center gap-2 flex-wrap">
                            {item.ingredients.map((ing) => (
                              <span
                                key={ing}
                                className="bg-[#d06224bf] rounded-[10px] px-3 py-0.5 text-white text-xs"
                              >
                                {ing}
                              </span>
                            ))}
                            <span className="text-gray-400 text-xs">{item.time}</span>
                            <span className="text-gray-400 text-xs">{item.portion}</span>
                          </div>
                        </div>
                        ...
                        <div className="flex flex-col items-end gap-2 shrink-0">
                          ...
                          <button
                            ...
                            className="h-[34px] px-5 bg-[#d06224] text-white text-sm font-medium rounded-[10px] hover:bg-[#b85520] transition"
                          >
                            Masak Lagi
                          </button>
                        </div>
          ...
          <div className="w-[280px] flex flex-col gap-4 shrink-0">
            ...
            <div className="bg-white rounded-[15px] shadow-sm p-4 flex flex-col gap-2">
              {resepSering.map((resep) => (
                ...
                >
                  <span className="text-sm font-semibold text-[#d06224] w-4 shrink-0">
                    {resep.rank}
                  </span>
                  <span className="flex-1 text-sm text-gray-700">{resep.title}</span>
                  <span className="text-xs font-semibold text-white bg-[#9f9b4a] rounded-[6px] px-2 py-0.5 shrink-0">
                    {resep.count}x
                  </span>
                </div>
              ))}
