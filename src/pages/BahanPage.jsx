...
export default function BahanPage() {
  ...
  if (location.state?.newItem) {
    ...
    // Bersihkan state agar tidak re-add saat refresh
  ...
  const [categories, setCategories] = useState(() => {
    ...
    if (!savedItems) return initial;
      ...
      // Gabungkan: ambil items dari localStorage, tapi image tetap dari initial
      const imageMap = {
  ...
  useEffect(() => {
    saveStorage("bahan_sortOrder", sortOrder);
  }, [sortOrder]);

  // Simpan items per kategori (tanpa image karena object JS)
  useEffect(() => {
    const itemsMap = {};
        ...
        <div className="flex flex-col md:flex-row gap-3 mb-5">
          <div className="flex flex-1">
            <input
              ...
              className="flex-1 h-[40px] rounded-l-full px-6 bg-white outline-none text-sm shadow-sm"
            />
            <button className="w-[50px] h-[40px] rounded-r-full bg-[#d06224] flex items-center justify-center shadow-sm">
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            <button
              onClick={handleSort}
              className={`h-[40px] px-6 rounded-full text-white font-Regular flex items-center justify-center gap-2 transition-colors
            ...
            <button
              ...
              className="h-[40px] px-6 rounded-full bg-[#d06224] text-white font-Regular flex items-center justify-center gap-2"
            >
          ...
          <div className="bg-white rounded-2xl p-5 shadow-sm">
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
              {filteredCategories.flatMap((category) => category.items).map((item) => (
                <div
                  key={item.id}
                  className="w-full rounded-xl overflow-hidden bg-white hover:shadow-md transition-shadow shadow-sm border border-gray-200"
                >
                  <div className="w-full h-[160px] overflow-hidden border-2 border-[#d06224] rounded-none">
