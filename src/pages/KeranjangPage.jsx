  ...
  export default function KeranjangPage() {
    ...
    const fetchList = useCallback(async () => {
      try {
        setLoading(true);
        setError(null);
        const resResponse = await shoppingListAPI.getAll(); // ← ganti api.getList()
        if (resResponse?.success && Array.isArray(resResponse.data?.shoppingList)) {
    ...
    const toggleItem = async (id) => {
      setItems((prev) => prev.map((item) => item._id === id ? { ...item, checked: !item.checked } : item));
      try {
        await shoppingListAPI.toggleItem(id); // ← ganti api.toggleItem
      } catch { fetchList(); }
    };
    ...
    const hapusSelesai = async () => {
      ...
      try {
        await shoppingListAPI.clearChecked(); // ← ganti api.clearChecked()
    ...
    const hapusItem = async (id) => {
      ...
      try {
        await shoppingListAPI.deleteItem(id); // ← ganti api.deleteItem
      } catch { fetchList(); }
    };
    ...
    const tambahItem = async () => {
      ...
      setActionLoading(true);
      try {
        await shoppingListAPI.addItem({ name: newItemName.trim() }); // ← ganti api.addItem
        await fetchList();
        setNewItemName("");
          ...
          {NAV_ITEMS.map((item) => {
            ...
            return (
              <button key={item.label} onClick={() => navigate(item.path)}
                ...
                {item.icon}<span className="leading-tight">{item.label}</span>
              </button>
            );
          })}
            ...
            <div className="flex-1 flex flex-col gap-4">
              ...
              <div className="grid grid-cols-3 gap-3 md:gap-4">
                {[
                  ...
                  <div key={stat.label} className="bg-white rounded-[15px] flex flex-col items-center justify-center py-4 md:py-6 shadow-sm">
                    <span className={`text-3xl md:text-5xl font-bold ${stat.color}`}>{stat.value}</span>
                  ...
                  filtered.map((item, idx) => (
                    ...
                    >
                      ...
                      <div className="flex-1 flex items-center gap-2 cursor-pointer" onClick={() => toggleItem(item._id)}>
                        ...
                        {item.checked && (
                          <span className="text-xs bg-[#f5c5ac] text-[#d06224] font-semibold px-2 py-0.5 rounded-full hidden sm:inline">
                            Sudah Beli
                          </span>
                        )}
                      </div>
